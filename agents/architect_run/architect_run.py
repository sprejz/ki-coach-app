...
hier ist nur der Prompt wirklich laufspezifisch, und der liegt jetzt als eine
zusammenhängende Datei neben dem Code statt als Kern+Zusatz verteilt.
"""
...
SPORT = "Laufen"
_PROMPT_PATH = Path(__file__).parent / "architect_run.md"


def run(*, athlete: dict, workout: dict, auftrag: dict, wetter_zeile: str = "",
        sport: str = "", model: str = HAIKU) -> dict:
    # `sport` wird hier nicht ausgewertet (dieses Modul ist schon laufspezifisch)
    # — der Parameter existiert nur für eine einheitliche Aufrufsignatur mit
    # dem generischen Fallback (agents/architect), den der Orchestrator per
    # Dispatch austauschbar aufruft.
    return call_agent(
