...
TRANSLATIONS = {
    "de": {
        ...
        "prompt_system": (
            ...
            "Bei GO: Übernimm die originale Workout-Beschreibung aus dem TrainingPeaks-Kontext exakt so wie sie ist.\n"
            "Bei MOD: KOPIERE die Schritte des originalen Workouts aus dem TrainingPeaks-Kontext und ändere NUR was geändert werden muss. "
            "Beispiele: 3×10min @ Schwelle → 2×8min @ Z3 (Intensität runter), 90min Z2 @ 240W → 60min Z2 @ 200W (kürzen + Watt runter). "
            "Behalte Aufwärmen/Hauptteil/Auslaufen-Struktur bei, ändere nur die betroffenen Werte.\n"
            "Wenn die Anpassung so grundlegend ist, dass die Original-Struktur komplett hinfällig ist "
            "(z.B. hartes Intervall-Training → reiner Regenerationstag): schreibe '⚠️ Einheit komplett umgestellt' "
            "in die erste Zeile, dann die neue Einheit.\n"
            "Format: kurze Stichpunkte, NUR was der Athlet tun soll, keine Begründungen. Max 5 Zeilen.\n\n"
    ...
    "en": {
        ...
        "prompt_system": (
            ...
            "For GO: Copy the original workout description from the TrainingPeaks context exactly as-is.\n"
            "For MOD: COPY the steps of the original workout from the TrainingPeaks context and change ONLY what needs to change. "
            "Examples: 3×10min @ threshold → 2×8min @ Z3 (lower intensity), 90min Z2 @ 240W → 60min Z2 @ 200W (shorter + lower watts). "
            "Keep the warm-up/main set/cool-down structure, only modify the affected values.\n"
            "If the adjustment is so fundamental that the original structure is completely obsolete "
            "(e.g. hard interval session → pure recovery day): write '⚠️ Einheit komplett umgestellt' "
            "on the first line, then write the new session.\n"
            "Format: short bullet points, ONLY what the athlete should do, no reasoning. Max 5 lines.\n\n"
