...
TRANSLATIONS = {
    "de": {
        ...
        "prompt_system": (
            ...
            "Bei MOD: ORIENTIERE DICH AN DER ORIGINALEN BESCHREIBUNG aus dem TrainingPeaks-Kontext. "
            "Behalte die Workout-Struktur (Aufwärmen / Hauptteil / Auslaufen) bei, passe aber Dauer, Intensität oder Umfang konkret an. "
            "Beispiel: 3×10min @ Schwelle wird zu 2×8min @ Z3, 90min Z2 wird zu 60min Z1-Z2. "
            "Nur wenn die nötige Anpassung so grundlegend ist, dass die originale Struktur keinen Sinn mehr ergibt "
            "(z.B. Intervall-Session → komplette Regeneration), erstelle eine neue Workout-Struktur.\n"
            "Format: kurze Stichpunkte, NUR was der Athlet tun soll, keine Begründungen, keine Erklärungen. Max 5 Zeilen.\n\n"
    ...
    "en": {
        ...
        "prompt_system": (
            ...
            "For MOD: BASE YOUR DESCRIPTION ON THE ORIGINAL from the TrainingPeaks context. "
            "Keep the workout structure (warm-up / main set / cool-down) but adjust duration, intensity or volume concretely. "
            "Example: 3×10min @ threshold becomes 2×8min @ Z3, 90min Z2 becomes 60min Z1-Z2. "
            "Only create a completely new workout structure if the required modification is so fundamental "
            "that the original structure no longer makes sense (e.g. interval session → full recovery).\n"
            "Format: short bullet points, ONLY what the athlete should do, no reasoning, no explanations. Max 5 lines.\n\n"
